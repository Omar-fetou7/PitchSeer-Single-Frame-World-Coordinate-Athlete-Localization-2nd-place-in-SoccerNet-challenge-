...
def main():
    ...
    for chunk_idx, image_chunk in enumerate(chunked(image_paths, args.batch_size), start=1):
        ...
        try:
            ...
            # Close the streaming generator explicitly so the torch.inference_mode()
            # context manager inside it is cleaned up while torch is still fully loaded.
            # Without this, cleanup runs at interpreter shutdown when torch may be
            # partially unloaded, causing AttributeError on torch._jit_internal.
            try:
                results.close()
            except Exception:
                pass
