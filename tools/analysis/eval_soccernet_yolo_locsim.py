...
def parse_args():
    ...
    parser.add_argument(
        '--ann-file',
        required=True,
        help='COCO-format annotation file (with camera_matrix / undist_poly / position_on_pitch).')
    parser.add_argument(
        '--img-prefix',
        required=True,
        help='Image directory for the split being evaluated.')
    parser.add_argument(
        '--output-dir',
        default='evaluation/yolo_detection',
        help='Directory for detections, pose predictions, metrics, and summary (relative to repo root).')
    parser.add_argument(
    ...
    parser.add_argument(
        '--metrics-name',
        default=None,
        help='Stem for the metrics JSON saved under <output-dir>/metrics/. '
             'Defaults to <checkpoint-stem>_yolo_best.')
...
def main():
    ...
    metrics_stem = args.metrics_name if args.metrics_name else f'{checkpoint_slug}_yolo_best'
    det_slug = f'{checkpoint_slug}__yolo_boxes'
    detector_json_path = det_dir / f'{det_slug}.json'
    pose_pred_json_path = pose_dir / f'{checkpoint_slug}_trainval.json'
