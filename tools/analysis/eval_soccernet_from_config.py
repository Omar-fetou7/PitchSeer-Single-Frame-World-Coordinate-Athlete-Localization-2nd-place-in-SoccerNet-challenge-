...
def main():
    ...
    if detector_cfg['type'] == 'oracle':
        command = [
            'conda', 'run', '-n', pose_cfg['conda_env'], '--no-capture-output',
    ...
    detector_command = [
        'conda', 'run', '-n', detector_cfg['conda_env'], '--no-capture-output',
    ...
    if eval_cfg.get('metrics_name'):
        pose_command.extend(['--metrics-name', str(eval_cfg['metrics_name'])])
    if eval_cfg.get('overwrite', True):
